...
async function endGame() {
  ...
  if (_mode === "multi") {
    ...
    // 호스트라면 방 상태를 finished로 변경
    const room = getCurrentRoom();
    if (room?.isHost) {
      await markGameFinished();
    }
    // 잠시 대기 후 결과 화면 (다른 플레이어 점수 동기화 여유)
...
// ─────────────────────────────────────────────
function playAgain() {
  location.href = `game-multiplication.html?mode=${_mode}${_roomCode ? "&roomCode="+_roomCode+"&gameId="+GAME_ID : "&gameId="+GAME_ID}`;
}

function goLobby() {
  if (_mode === "multi") leaveRoom().finally(() => { location.href = "index.html"; });
