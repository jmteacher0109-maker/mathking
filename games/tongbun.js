...
(function () {
  ...
  let connectedCnt = 0;
  let activeDrags  = {};
  let hitRadius    = 32;   // 점 충돌 반경 (보드 크기에 따라 갱신)
  ...
  function _generateBoard() {
    _cancelAllDrags();  // 판 전환 타이밍 버그 방지
    ...
    cells.forEach((cell, i) => {
      ...
      btn.innerHTML = `<div class="tb-frac"><div class="tb-top">${fracs[i].n}</div><div class="tb-bot">${fracs[i].d}</div></div>`;
  ...
  /**
   * 혼동 감지
   * (1) 다른 쌍끼리 분모가 같으면 학생이 헷갈림  예) 3/6 ↔ 2/6
  ...
  function _isConfusing(fracs) {
    ...
    for (const f of fracs) {
      ...
      seen.add(key);
      if (denomMap[f.d] !== undefined && denomMap[f.d] !== f.pairId) return true;
  ...
  function _onMove(e) {
    ...
    for (const dot of drag.dots) {
      ...
      if (_ptSegDist(dot, p1, p2) < hitRadius) {
        // pairId 일치 AND 교차곱 수학 검증 (이중 안전장치)
        const samePair = drag.startPairId === dot.pairId;
        const mathOK   = drag.startN * dot.d === dot.n * drag.startD;
        if (!dot.connected && samePair && mathOK) _complete(drag, dot.el, e.pointerId);
        else _fail(drag, e.pointerId);
        return;
      }
    }
  ...
  function _fail(drag, pid) {
    ...
    setTimeout(() => b.classList.remove('tb-shake'), 400);
  ...
  function _complete(drag, targetEl, pid) {
    ...
    const seg = { startId: drag.startId, endId: targetEl.dataset.id, points: [...drag.points], pathEl: drag.pathEl, scoreGained: gained };
    paths.push(seg);
    drag.pathEl.addEventListener('pointerdown', ev => _undo(ev, seg));

    connectedCnt++;
    if (connectedCnt >= PAIRS) setTimeout(_generateBoard, 500);
    delete activeDrags[pid];
  }
  ...
  function _intersect(A, B, C, D) { return _ccw(A,C,D) !== _ccw(B,C,D) && _ccw(A,B,C) !== _ccw(A,B,D); }
  ...
  function _injectStyles() {
    ...
    s.textContent = `
      ...
      .tb-frac { display:flex; flex-direction:column; align-items:center; line-height:1.05; }
      .tb-top  { border-bottom:2px solid currentColor; padding:0 4px; margin-bottom:2px; }
      .tb-bot  { padding:0 4px; }

      .tb-dot.tb-active    { background:#22d3ee; color:#000; transform:scale(1.18); border-color:#fff; box-shadow:0 0 16px #22d3ee99; }
      .tb-dot.tb-connected { background:#0d8fa3; color:#fff; border-color:#22d3ee; pointer-events:none; transform:scale(.82); opacity:.85; }
      ...
      .tb-drag {
        ...
        fill: none; opacity: .65; pointer-events: none;
        stroke-dasharray: 10 5;
        animation: tb-flow .5s linear infinite;
      }
      @keyframes tb-flow { to { stroke-dashoffset: -30; } }
